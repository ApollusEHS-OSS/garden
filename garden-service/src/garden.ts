  ...
  static async factory<T extends typeof Garden>(
    ...
    if (clientAuthToken && !opts.noPlatform) {
      if (!cloudDomain || !projectId) {
        ...
        if (!projectId) {
          errorMessages.push(deline`
            ${chalk.bold("project.id")} is not set in your project-level ${chalk.bold("garden.yml")}. Please use the
            ${chalk.bold("garden set project")} command to set this value, or paste it from the platform's web UI
            for your project.
          `)
        }
